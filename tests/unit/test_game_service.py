...
def mock_llm() -> AsyncMock:
    ...
    mock_service = AsyncMock(spec=LLM)
...
class TestGameInit:
    ...
    def test_game_task_is_none_on_init(self, svc: Game) -> None:
        """Test no background task exists before begin_game is called."""
        assert svc._game_task is None, '_game_task must be None on init'

    def test_agent_manager_exists(self, svc: Game) -> None:
        """Test Game has an embedded AgentManager instance."""
...
class TestGameStop:
    ...
    async def test_stop_cancels_active_game_task(self, svc: Game) -> None:
...
class TestGameBeginGame:
    """Tests for Game.begin_game."""

    async def test_begin_game_raises_when_game_already_active(
        self, svc: Game, monkeypatch: MonkeyPatch
    ) -> None:
...
class TestGameGetGameState:
    """Tests for Game.get_game_state."""

    def test_returns_current_state(self, svc: Game) -> None:
    ...
    def test_returns_defensive_copy_of_alive_list(self, svc: Game) -> None:
...
class TestGameSubmitHostDecision:
    """Tests for Game.submit_host_decision."""

    def test_stores_the_decision(self, svc: Game) -> None:
    ...
    def test_sets_host_decision_event(self, svc: Game) -> None:
...
class TestGameSubscribeMessages:
    """Tests for Game.subscribe_messages."""

    async def test_replays_message_history(self, svc: Game) -> None:
    ...
    async def test_streams_newly_arrived_messages(self, svc: Game) -> None:
...
class TestGameForceStopAgent:
    """Tests for Game.force_stop_agent."""

    async def test_removes_agent_from_alive(self, svc: Game) -> None:
...
class TestGameEventBusIntegration:
    """Tests for EventBus integration in Game."""
    ...
    def svc_with_event_bus(
        ...
        """Game with EventBus and mocked dependencies."""
        monkeypatch.setattr(
        ...
        service = Game(settings, llm=mock_llm, event_bus=mock_event_bus)
        return service, mock_event_bus

    async def test_publishes_message_events(
        self, svc_with_event_bus: tuple[Game, MagicMock]
    ) -> None:
        """Test Game publishes MESSAGE events when agents speak."""
    ...
    async def test_publishes_vote_events(
        ...
        """Test Game publishes VOTE events when votes received."""
        svc, event_bus = svc_with_event_bus
    ...
    async def test_publishes_state_change_events(
        ...
        """Test Game publishes STATE_CHANGE events on phase transitions."""
        svc, event_bus = svc_with_event_bus
    ...
    async def test_ask_agent_publishes_answer_event(
        self,
        svc_with_event_bus: tuple[Game, MagicMock],
        monkeypatch: MonkeyPatch,
    ) -> None:
